			...
			$.ajax({
				...
				if (typeof data.result != 'undefined') {
					...
					$.each(data.result, function(i,item){			  
						...
						var img_device = atob(item.StrParam2);
						var track = atob(item.StrParam1);
						...
						xhtm+=  '\t      <td id="status">ST' + TranslateStatus(item.Status) + '</td>\n' +
							'\t      <td id="internalstate">IS' + item.InternalState + '</td>\n' +
							'\t      <td id="lastupdate">LU' + item.LastUpdate + '</td>\n';
...
//						xhtm+=  '<td><div style="margin-left:0px;" width="90%" class="dimslider" id="slider" data-idx="' + item.idx + '" data-type="norm" data-maxlevel="' + item.MaxDimLevel + '" data-isprotected="' + item.Protected + '" data-svalue="' + item.LevelInt + '"></div><br></td>';
						xhtm+=  '\t<td><div style="margin-left:70px;" class="dimslider" id="slider" data-idx="' + item.idx + '" data-type="norm" data-maxlevel="' + item.MaxDimLevel + '" data-isprotected="' + item.Protected + '" data-svalue="' + item.LevelInt + '"></div><br></td>';
						// Bottom buttom area
						xhtm+=  '<td>';
					  	if (item.Favorite == 0) {
							xhtm+=      
